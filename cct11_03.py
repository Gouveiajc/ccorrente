...
def alterar_registro(tree, atualizar_grid_callback=None):
    ...
    codigo, descricao, tipo, obs = tree.item(item)["values"]

    janela = tk.Toplevel(tree.master)
    janela.title("Alteração de Tipo de Natureza")
    ...
    def salvar():
        ...
        novo_obs = entry_obs.get().strip()

        conn = cct00_0.conectar()

        # Gravação
        cct00_0.alterar_registro(conn, codigo, desc, novo_tipo, novo_obs)
        conn.close()
